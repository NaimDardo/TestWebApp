...
const AppProfile = () => {
  ...
  const SubmitNewSettings = (e) =>{
	e.preventDefault()
	let current_user_id = JSON.parse(localStorage.getItem("userDetails"))['id']
	const elements = e.target.elements
	let data = {}
	for (let item in elements) {
		if (isNaN(item) && !['item','namedItem','length','gridCheck'].includes(item)){
  ...
  return (
    <Fragment>
      <PageTitle  activeMenu="My Profile" />
              ...
              <div className="profile-info">
                ...
                <div className="profile-details">
                  <div className="profile-name px-3 pt-2">
                    <h4 className="text-primary mb-0">{userDet.name}</h4>
                    <p>{userDet.role === "DOC"? "DOCTOR": userDet.role === "HOS"? "HOSPITAL AGENT": userDet.role === "DN"? "Nursery Admin":""}</p>
                  </div>
                  <div className="profile-email px-2 pt-2">
                    <h4 className="text-muted mb-0">{userDet.email}</h4>
                    <p>Email</p>
                  </div>
                ...
                <div className="custom-tab-1">
						<ul className="nav nav-tabs">
							...
							<li className="nav-item">
								<Link to="#profile-settings" data-toggle="tab" onClick={() => setActiveToggle("setting")} className={`nav-link active show`}>Setting</Link>
							</li>
						</ul>
								...
								<div className="settings-form">
									...
									<form onSubmit={SubmitNewSettings}>
										...
										<div className="form-row">
											<div className="form-group col-md-6">
												<label>Address</label>
												<input type="text" placeholder="1234 Main St" className="form-control" name="Adress"/>
											</div>
